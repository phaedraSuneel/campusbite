# -*- encoding: utf-8 -*-
# stub: vestal_versions 2.0.0 ruby lib

Gem::Specification.new do |s|
  s.name = "vestal_versions"
  s.version = "2.0.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Steve Richert", "James O'Kelly", "C. Jason Harrelson"]
  s.date = "2014-01-21"
  s.description = "Keep a DRY history of your ActiveRecord models' changes"
  s.email = ["steve.richert@gmail.com", "dreamr.okelly@gmail.com", "jason@lookforwardenterprises.com"]
  s.homepage = "http://github.com/laserlemon/vestal_versions"
  s.licenses = ["MIT"]
  s.rubygems_version = "2.2.2"
  s.summary = "Keep a DRY history of your ActiveRecord models' changes"

  s.installed_by_version = "2.2.2" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<activerecord>, ["< 5", ">= 3"])
      s.add_runtime_dependency(%q<activesupport>, ["< 5", ">= 3"])
      s.add_development_dependency(%q<bundler>, ["~> 1.0"])
      s.add_development_dependency(%q<rake>, ["~> 10.0"])
    else
      s.add_dependency(%q<activerecord>, ["< 5", ">= 3"])
      s.add_dependency(%q<activesupport>, ["< 5", ">= 3"])
      s.add_dependency(%q<bundler>, ["~> 1.0"])
      s.add_dependency(%q<rake>, ["~> 10.0"])
    end
  else
    s.add_dependency(%q<activerecord>, ["< 5", ">= 3"])
    s.add_dependency(%q<activesupport>, ["< 5", ">= 3"])
    s.add_dependency(%q<bundler>, ["~> 1.0"])
    s.add_dependency(%q<rake>, ["~> 10.0"])
  end
end
